...
class Drawing:
    ...
    @staticmethod
    def rectangle(
        image, top_left_point, bottom_right_point, color=Color.black, thickness=2
    ):
        ...
        return cv2.rectangle(
            image.copy(), (x1, y1), (x2, y2), color, thickness
        )  # (0, 255, 0) is the color in BGR format, and 2 is the line thickness
    ...
    def draw_lines(lines, img, output_path, lines_all=None, thickness=1):
        ...
        if lines_all is not None:
            for line in lines_all:
                ...
                drawn_img = Line.draw_line(
                    x1, y1, x2, y2, drawn_img, thickness, Color.white
                )
                # cv2.line(drawn_img, (int(x1), int(y1)), (int(x2), int(y2)), Color.red, thickness)
        for line in lines:
            x1, y1, x2, y2 = line.ravel()
            x1, y1, x2, y2, x3, y3, x4, y4 = compute_intersection_with_block_boundaries(
                np.array((x1, y1)), np.array((x2, y2)), img
            )
            if (
                x1 is None
        ...
        drawn_img = np.where(drawn_img > 255, 255, drawn_img).astype(np.uint8)
...
def resize_image_using_pil_lib(
    im_in: np.array, height_output: object, width_output: object, keep_ratio=True
) -> np.ndarray:
    """
