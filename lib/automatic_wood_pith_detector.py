...
def pclines_postprocessing(
    img_in, Lof, ransac_outlier_th=0.03, debug=False, output_folder=None
):
...
def read_label(label_filename, img):
    ...
    if label.shape[0] > 1:
        label = label.iloc[0]
    cx, cy, w, h = (
        int(label[1] * img.shape[1]),
        int(label[2] * img.shape[0]),
        int(label[3] * img.shape[1]),
        int(label[4] * img.shape[0]),
    )
    return cx, cy, w, h
